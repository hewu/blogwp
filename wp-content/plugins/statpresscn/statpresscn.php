...
<div class='wrap'>
    ...
    or comment at <a href='http://heart5.com/?page_id=629#comments' target='_heart5'>
        StatPressCN Info</a> for debug. All suggestion is gladly appreciated!
    <table>
        ...
        <tr><td>php datetime</td><td><?php echo "date() is ".date("Ymd His")
                        ...
                        .", localtime() is ".date("Ymd His",time())
                    ;?></td></tr>
...
<div class='wrap'><h2><?php _e('Support','statpresscn'); ?></h2>
    <IFRAME width="100%" HEIGHT="1600px"  FRAMEBORDER=0 SCROLLING="auto" SRC="http://heart5.com/?page_id=629">
        ...
        <a href='http://heart5.com/2008/09/12/statpresscn.html' target='heart5'><?php _e('Developing page','statpresscn'); ?></a>
...
<div class='wrap'>
    ...
    <form method=post>
        <table>
            ...
            <tr>
                <td><?php _e('Automatically delete visits older than','statpresscn'); ?>
                    <select name="statpress_autodelete">
                        <option value="" <?php if(get_option('statpress_autodelete') =='' ) print "selected"; ?>><?php _e('Never delete!','statpresscn'); ?></option>
                        <option value="1 week" <?php if(get_option('statpress_autodelete') == "1 week") print "selected"; ?>>1 <?php _e('week','statpresscn'); ?></option>
                        <option value="2 weeks" <?php if(get_option('statpress_autodelete') == "2 weeks") print "selected"; ?>>2 <?php _e('weeks','statpresscn'); ?></option>
                        <option value="1 month" <?php if(get_option('statpress_autodelete') == "1 month") print "selected"; ?>>1 <?php _e('month','statpresscn'); ?></option>
                        <option value="3 months" <?php if(get_option('statpress_autodelete') == "3 months") print "selected"; ?>>3 <?php _e('months','statpresscn'); ?></option>
                        <option value="6 months" <?php if(get_option('statpress_autodelete') == "6 months") print "selected"; ?>>6 <?php _e('months','statpresscn'); ?></option>
                        <option value="1 year" <?php if(get_option('statpress_autodelete') == "1 year") print "selected"; ?>>1 <?php _e('year','statpresscn'); ?></option>
                    </select></td>
...
function iriStatPressMobiler() {
    $mobileosset = "('Windows CE','iPhone','iPod','Nokia','SonyEricsson','SAMSUNG','NEC','Moto','Dopod','Amoi','Symbian','Lenovo','KBT_N650B')";
    $mobilebrowserset = "('Opera Mini','UCWEB','J2ME','MAUI WAP Browser')";
...
<div class='wrap'><h2><?php _e('Search','statpresscn'); ?></h2>
    <form method=get><table>
        ...
        </table><!-- It's strange that hte page value should be spc-search, and not others. -->
        ...
        # query builder
            ...
            if($groupby != '') { print "<th scope='col'>".__('Count','statpresscn')."</th>"; }
...
*/
        ...
        if($out_url == '') { $out_url=__('Home','statpresscn'); }
        elseif(preg_match("@^paged=([\d]+)$@i",$out_url,$matches)) {
            $out_url=__('Home','statpresscn')." ".$matches[1];
        }elseif(preg_match("@^cat=([\d]+)(?:&paged=([\d]+))?$@i",$out_url,$matches)) {
            $out_url=__('Category','statpresscn').": ".get_cat_name($matches[1])." ".$matches[2];
        }elseif(preg_match("@^m=([\d]{8})(?:&paged=([\d]+))?$@i",$out_url,$matches)) {
            $out_url=__('Calendar','statpresscn').": ".$matches[1]." ".$matches[2];
        }elseif(preg_match("@^m=([\d]{6})(?:&paged=([\d]+))?$@i",$out_url,$matches)) {
            $out_url=__('Archive','statpresscn').": ".$matches[1]." ".$matches[2];
            ...
            $out_url=__('Search','statpresscn').": ".urldecode(substr($matches[1],2));
        }elseif(preg_match("@^(feed=rss2&)?p=([\d]+)$@i",$out_url,$matches)) {
            $post_id_7 = get_post($matches[2], ARRAY_A);
            $out_url = $post_id_7['post_title'];
            if(strlen($matches[1])>0) {
                $out_url = __("Feed","statpresscn").": ".$out_url;
            }
...
function heart5_get_post_name_or_id($url,$per_struct) {
    ...
    if(preg_match($per_str_reg,$url,$matches)) {
        if(key_exists("post_id",$matches)) {
            $result = $matches["post_id"];
        }else if(key_exists("postname",$matches)) {
...
function iriGetSE($referrer = null) {
    ...
    foreach($lines as $line_num => $se) {
        ...
        if(stripos($nome,"Google") !== false & key_exists("prev",$pieces)) {
    ...
    <form method=post>
        <table>
            ...
            <tr>
                ...
                <td><input type=checkbox name='statpress_updatesets_longth' value='checked'> <?php _e('Update all sets based longth you configure','statpresscn');?></td>
            </tr>
            <tr>
...
function iri_StatPress_TopPosts($limit=5, $long='', $showcounts='checked') {
    ...
    if ($long !='') {
        if(strtotime($long)) {
            $tmpdate = date("Ymd",strtotime("-".$long));
    ...
    foreach ($qry as $rk) {
        ...
        $res.="<li><a href='".heart5_config_url($rk->urlrequested)."'>".$post_title."</a></li>\n";
...
<ul>
<li>Current page is visited %thistotalvisits% times</li>
<li>Now there are %visitorsonline% friends visiting this blog, %visits% today, %yesterdayvisits% yesterday,
and %totalvisits% totally</li>
<li>The posts have been viewed %pagevisits% times today, %yesterdaypagevisits% yesterday, and %totalpagevisits% totally</li>
<li>%feeds% friends have subscribed this blog<li>
<li>The TopPost is %toppost%, TopBroswer is %topbrowser%, TopOS is %topos%</li>
<li>You come from %comefrom%, your broswer is %browser%, OS is %os%</li>
<li>This blog have %blogtotalpost% posts, %blogtotalpage% pages, %blogtotalcomment% comments, and %blogtotalcommentor% commentors.</li>
</ul>";
...
function iri_update_manuplate_sets_autodelete() {
    ...
    $table_name = $wpdb->prefix . "statpress";
    $wpdb->show_errors();
    print "".__("Delete sets if needed","statpresscn").":";
    if(get_option('statpress_autodelete') != '') {
        $t = gmdate("Ymd",strtotime('-'.get_option('statpress_autodelete')));
        $setcount = $wpdb->query( "DELETE FROM " . $table_name . " WHERE date < '" . $t . "'");
        print $setcount." sets are deleted. ";
        //做了删除记录的操作，需要同步更新归档数据
        $qry = $wpdb->get_results("SELECT date FROM $table_name ORDER BY date LIMIT 1;");
        if((strlen(get_option("statpress_date_first_sets"))==0)||(get_option("statpress_date_first_sets")!=$qry[0]->date)) {
            update_option("statpress_date_first_sets",$qry[0]->date);
        }
    }
...
function heart5_archive_datasets() {
    ...
    $vyesterday = gmdate("Ymd",$timestamp);
    $timestamp = strtotime("-1 month");
    $vlastmonth = gmdate("Ym",$timestamp);
    $timestamp = strtotime("-1 year");
    $vlastyear = gmdate("Y",$timestamp);
