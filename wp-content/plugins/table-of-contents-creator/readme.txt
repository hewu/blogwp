...
1. An example table of contents with collapsed children, square icon theme and one selected summaries shown.
1. An example table of contents without icons.
1. An example table of contents using the haddrawn icon theme and expanded option menu.
...
* Add Spanish (castilian) translation by Fernando J. Echevarrieta (echeva) [http://3d.echeva.com/]
...
* Updated Italian translation to include Summarizer options thanks to Carlo Politi [http://www.carlopoliti.net/]
...
* Fixed bug in tocc_plugin_options() function that stopped the admin page loading on some systems. Thank you very much Michael E. Chancey Jr. [http://michael.chanceyjr.com] for pointing out the bug and offering a fix.
...
* Added Italian language file by Carlo Politi [http://carlopoliti.net]
...
* Short code override to exclude posts from listed categories.
* Add option to include TOCC on 404 page.
* Sort by user defined tag.
* Add option for maximum number of links per page (split list over multiple pages). Set to 0 or -1 to disable (toc pagination).
* Add page_include and blog_include options that are settable only via the short code. This allows specific pages or categories to be shown on selected pages. Currently, this can be done but you need to exclude all other pages.
