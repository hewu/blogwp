...
define('TOCC_VERSION', '1.6.3.1'); // used for upgrade options and display purposes
...
function tocc_build_toc(&$tocc_options, $dontModHeader = false) {
	...
	if($tocc_options['show_help']) {
		$tocc .= '&nbsp;<a href="#" title="' . $tocc_options['help_tooltip'] .'" class="tocc_help_icon"></a>'
				. '<p class="tocc_help_text">' . $tocc_options['help_text'] . '<br /><br /></p>' . PHP_EOL; 
	}
...
function tocc_display_posts($postCount, $allowHideRoot, $depth, &$tocc_options) {
	...
	// show all posts in one big list
		...
		foreach ($posts as $post) {
			...
			$content .= tocc_create_link($post, 'blog_post', false, false, false, get_permalink($post->ID), $post->post_title
	...
	if (get_option('tocc_version')) {
		...
		if (get_option('tocc_version') != constant('TOCC_VERSION')) {
			...
			if ($old_ver < 1006003) {
				// add new options
		   	$tocc_options = get_option('tocc_options');
				$tocc_options['show_help'] = $GLOBALS['tocc_defaults']['show_help'];'checked';
...
function tocc_plugin_options() {
	...
	printf(__('To use the plugin, insert the trigger text %s into an existing page. The trigger will be auomatically replaced with a complete site wide table of content. You may also use the <code>create page</code> button below.', 'tocc'), ('<strong>' . htmlspecialchars(constant('TOCC_TAG')) . '</strong>'));
	...
	tocc_option($tocc_options, __('Show Help Icon', 'tocc'), __('Dislays a small icon in the top left corner that when clicked will display instructions to the user.', 'tocc'), 'show_help', 'checkbox');
	tocc_option($tocc_options, __('Help Icon ToolTop', 'tocc'), __('Tooltip text to be displayed when the user hovers the mouse over the help icon.', 'tocc'), 'help_tooltip', 'text', 40);
	tocc_option($tocc_options, __('Help Text', 'tocc'), __('Help text to display when the user clicks the help icon.', 'tocc'), 'help_text', 'textarea', '65,4');
	...
	echo PHP_EOL . '<h4>' . __('Acknowledgments', 'tocc') . '</h4>' . PHP_EOL
		...
		. '<a href="http://photomentors.com/">Steadman Uhlich</a> - ' . __('Major version pre-release validator', 'tocc') . PHP_EOL
		. '</div>' . PHP_EOL;
