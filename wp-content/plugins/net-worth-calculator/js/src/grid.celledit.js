...
$.jgrid.extend({
			...
			if ($t.p.colModel[iCol].editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$("input, select, textarea",cc).bind("keydown",function(e) { 
					...
					if (e.keyCode === 13) {//Enter
						$($t).jqGrid("saveCell",iRow,iCol,true);
					}
					// GROX END
					if (e.keyCode == 9)  {
	...
	// GROX
	saveCell : function (iRow, iCol, enter){
	// GROX END
		return this.each(function(){
			var $t= this, fr;
			//alert(enter);
			if (!$t.grid || $t.p.cellEdit !== true) {return;}
			//alert( $t.p.savedRow.length );
			...
			if(fr != null) {
				...
				// but we want it to always save... so:
				//if (v2 != $t.p.savedRow[fr].v){
				var save = false;
				if(nm == 'asset') {
					save = v2 != $t.p.savedRow[fr].v;
				} else {
					save = true;
				}
	...
	// GROX START
	saveOrder : function(rowid, successfunc, aftersavefunc) {
		return this.each(function(){
		var $t = this, nm, tmp={}, tmp2={}, editable, fr, cv, ind, url;
		...
		if (url) {
			...
			$.ajax($.extend({
				...
				complete: function(res,stat){
					$("#lui_"+$t.p.id).hide();
					if (stat === "success"){
						if( $.isFunction(successfunc) ) { successfunc(rowid,res); }
						if($t.p.autoencode) {
	...
	nextCell : function (iRow,iCol) {
		return this.each(function (){
			...
			if(nCol !== false) {
				// GROX
				if($t.p.colModel[nCol].name == 'private') {
					//$($t).jqGrid("saveCell",iRow,iCol);
					//alert($t.p.colModel[iCol].value);
					$($t).jqGrid("saveCell",iRow,iCol);
					//$($t).jqGrid('saveRow', $t.rows[iRow].id, function() { alert('test');});
					//$t.grid.emptyRows();
					//$t.grid.populate();
				} else {
