...
//Editing
					...
					if(keys===true) {
						$(ind).bind("keydown",function(e) {
							...
							if (e.keyCode === 13) {
								...
								$($t).jqGrid("saveRow",rowid,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc );
								return false;
							}
							e.stopPropagation();
						});
			...
			if (url == 'clientArray') {
				...
				$.ajax($.extend({
					...
					complete: function(res,stat){
						...
						if (stat === "success"){
							var ret;
							if( $.isFunction(successfunc)) { ret = successfunc(res);}
							else ret = true;
							if (ret===true) {
								if($t.p.autoencode) {
									$.each(tmp,function(n,v){
	...
	// GROX START
	addNewMonth : function(rowid, successfunc, aftersavefunc) {
		return this.each(function(){
